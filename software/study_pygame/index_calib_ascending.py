...
class Calibration_ascending:
    def __init__(self):
        ...
        self.vib_lvl = 0
        self.step = config.index_trials_thresholds[config.index_calib_done_trials - 1] / 4
        ...
        self.counter = 1
    def run(self):
        print(f'Starting the index #{config.index_calib_done_trials} ascending calibration...')
        ...
        while len(self.vib_lvl_history) < config.index_max_nb_trials and len(self.changing_points) < config.index_max_changing_points:
            ...
            if config.index_calib_done_trials == 2:
                noise_lvl = round(config.wrist_threshold * config.sr_coeff * 1000)
                self.ser_haptid.write(f'{noise_lvl}'.encode())
        ...
        config.index_trials_thresholds[config.index_calib_done_trials - 1] = round(config.index_trials_thresholds[config.index_calib_done_trials - 1], 3)
